import type { ExternalAffairsOverview } from "@/data/external-affairs";

export function EaOverview({ overview }: { overview: ExternalAffairsOverview }) {
  return (
      ...
      <div className="mb-10 grid grid-cols-2 gap-px overflow-hidden rounded-xl bg-auf-border sm:grid-cols-4">
        {overview.highlights.map((h) => (
          <div key={h.label} className="bg-navy px-4 py-5 text-center">
            <div className="font-display text-xl font-semibold text-gold-light sm:text-2xl">
              {h.value}
            </div>
